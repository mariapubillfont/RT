...
if 0:
    theta_i_y = [59.578, 58.55, 57.68, 57, 56.45, 56.07 , 55.8, 55.65, 55.55, 55.51, 55.509, 55.508]
    const = 316
    thy = np.array(theta_i_y)
    f = interp1d(Array, theta_i_y, kind='cubic')
    xnew = (np.linspace(0, len (Array) , num=1001, endpoint=True))
    fig = plt.figure()
    plt.plot(np.transpose(xnew), f(xnew))
    plt.show()
    
if 1:    
    theta_i_y = [43.6, 40.71, 37.85, 35.4, 33.25, 31.35, 29.7, 28.25, 26.9, 25.85, 24.9, 24.05]
    const = 258.2  
    x = np.linspace(0, len (Array) , 12)
    f = interp1d(x, theta_i_y, kind = 'cubic')
    xnew = (np.linspace(0, len (Array) , num=1001, endpoint=True))
    fig = plt.figure()
    plt.plot(x, theta_i_y, 'o')
...
if 0:
    ...
    theta_i_y = [[15.5, 12.3, 9.25, 6.5, 3.9, 1.155, -1.2, -3.9, -6.5, -9.25, -12.3, -15.5]]
    const = 191.7     
    
# theta_i_x_arr = np.zeros((len(theta_i_y), len(theta_i_y[0])))
# for i in range(len(theta_i_y)):
#     theta_i_x_arr[i] = np.deg2rad([90 -  x for x in theta_i_y[i]]) #incident angle with respect to x ax
# angle_out = []
# m_max = 10000
...
# for j in range(0,len(theta_i_x_arr)):
...
#         critical = getTheta_i_max(m_n, m_n2, theta_i_y[j][i]) #calulate the critical angle 
#         if critical > theta_i_x and theta_i_y[j][i] > 0 or critical < theta_i_x and theta_i_y[j][i] < 0 : 
...
#         phi_a[j][i] = -phi_i + const #50 is an arbitrary constant to center the phase to 0
